...
| any non-terminal → `HandedOff`                                                         | integration-controller   | Human closed the tracking issue (in review: PR still open)           |
...
unmerged → `Failed`, still open → `HandedOff`. A PR close from a repository other than the recorded one — renamed or
transferred since — is confirmed against the recorded PR the same way. Either close is kept on the finding as
`ReviewClosePending` until GitHub answers, because GitHub never redelivers a webhook it has already delivered.
