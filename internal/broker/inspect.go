...
// MCP server, a container, or a Files API reference anywhere in the tree.
// An empty body passes (a listing has none); a body that is not a JSON
// object is refused.
func inspectBody(body []byte) (inspection, error) {
	...
	if err := dec.Decode(&top); err != nil {
		return insp, fmt.Errorf("request body is not a JSON object")
	}
	...
	if tools, ok := top["tools"].([]any); ok {
		for _, t := range tools {
			...
			if typ, _ := tool["type"].(string); deniedToolType(typ) {
				return insp, fmt.Errorf("server tool %q is not brokered", typ)
			}
		}
	}
	if err := walkForFiles(top); err != nil {
		return insp, err
	}
	insp.model, _ = top["model"].(string)
	if n, ok := top["max_tokens"].(json.Number); ok {
		insp.maxTokens, _ = n.Int64()
	}
	return insp, nil
}
...
// walkForFiles refuses a file_id key or a source of type "file" at any depth:
// the Files API is never forwarded, so a reference into it must not be
// either.
func walkForFiles(v any) error {
	switch x := v.(type) {
	case map[string]any:
		if _, ok := x["file_id"]; ok {
			return fmt.Errorf("file references are not brokered")
		}
		if src, ok := x["source"].(map[string]any); ok {
			if typ, _ := src["type"].(string); typ == "file" {
				return fmt.Errorf("file references are not brokered")
			}
		}
		for _, child := range x {
			if err := walkForFiles(child); err != nil {
				return err
			}
		}
	case []any:
...
// bedrockPrefix is the inference-profile prefix bedrock ids carry.
var bedrockPrefix = regexp.MustCompile(`^([a-z]{2,4}\.)?anthropic\.`)
...
// bareModel normalizes an entry or a wire id: lower-case, the part after the
// last "/" (a canonical vendor prefix or an ARN's resource), and without a
// bedrock "<geo>.anthropic." prefix.
func bareModel(id string) string {
	id = strings.ToLower(strings.TrimSpace(id))
	if i := strings.LastIndex(id, "/"); i >= 0 {
		id = id[i+1:]
	}
	return bedrockPrefix.ReplaceAllString(id, "")
}
