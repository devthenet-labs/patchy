...
// # Aliases
//
// The legacy approve comment ("/approve", or the Integration's configured
// spec.github.issues.approveComment) parses as the verb approve with
// Command.Alias set to the form used. It is matched exactly as the Finding
// webhook handler has always matched it, so moving that handler onto this
// package cannot change which comments approve a Finding: the whole comment,
// trimmed, must equal the alias or start with the alias and a space; the
// match is case-sensitive; and the note is the rest of the comment, trailing
// lines included. The one deliberate difference is in the note, which is
// sanitised as above where the handler kept it verbatim apart from the 1 KiB
// cut. The /patchy grammar is tried first, so a configured alias that itself
// starts with the /patchy prefix is shadowed by it.
//
...
// whether the commenter may issue it are all the caller's decisions, as is
// whether to honour an alias on a given surface. The package makes no GitHub
// or Kubernetes call and imports neither; the verb names come from
// internal/action, the vocabulary the status page and the CLI share.
package command
