...
The investigation (attempt 2) stopped before the agent started: the image failed the agent's preflight
(`image_incompatible`):
```text
...
The image must be glibc-based, have `bash`, `sh` and `git` on its `PATH`, and let uid 65532 run them.
`patchy check image ghcr.io/acme/go-env:1.26 --run` runs the same preflight in a local container.
